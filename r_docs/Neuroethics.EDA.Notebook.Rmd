...
Self.Report <- cbind(survey[,"user_id"], survey[,181:187])
...
colnames(survey)[190] <- "time2completion"
...
MCList <- list(
  ...
  Murder <- MC[,4],
  Stealing <- MC[,5],
  ForcedSterilization <- MC[,6]
)

TEST = do.call(rbind, MCList)
colnames(TEST) = survey[,"user_id"]
...
#write.csv(Neuroethics_Judgement,'Cleaned_Pilot.02.csv')
...
```{r overall trial distribution for risk}

```

```{r subject wise odds of Yes}
...
TEST = dplyr::select(Neuroethics_Judgement, user_id , condition ,  Risk_Beta , Gain_Beta , Interaction_Beta, trial_index )

TEST = dplyr::select(Neuroethics_Judgement, user_id , condition ,risk, experimental_treatment_selected )

TEST = distinct(Neuroethics_Judgement,user_id, condition, Risk_Beta, Gain_Beta, Interaction_Beta, trial_index)


data_long = melt(TEST, 
                 id.vars=c("user_id", "condition", "trial_index"))
summary(data_long)

TEST$experimental_treatment_selected <- as.numeric(TEST$experimental_treatment_selected)
TEST$trial_index <- as.numeric(TEST$trial_index)
...
TEST[, paste("ot", 1:3, sep="")] <- 
  t[TEST$risk, 1:3]


m.1 <- glmer(experimental_treatment_selected ~ (ot1 + ot2 +ot3) +
               (1 + ot1 + ot2 +ot3 || user_id) + 
               (0 + ot1 + ot2 +ot3 || user_id:condition),
               control = glmerControl(optimizer = "bobyqa"),
               data = TEST, family = binomial)

m.2 <- glmer(experimental_treatment_selected ~ (ot1 + ot2 +ot3) + condition +
               (1 + ot1 + ot2 +ot3 || user_id) + 
               (0 + ot1 + ot2 +ot3 || user_id:condition),
               control = glmerControl(optimizer = "bobyqa"),
               data = TEST, family = binomial)

m.3 <- glmer(experimental_treatment_selected ~ (ot1 + ot2 +ot3)*condition +
               (1 + ot1 + ot2 +ot3 || user_id) + 
               (0 + ot1 + ot2 +ot3 || user_id:condition),
               control = glmerControl(optimizer = "bobyqa"),
               data = TEST, family = binomial)
...
ggplot(TEST, aes(risk, experimental_treatment_selected))+
  stat_summary(fun.data= mean_se, geom = "pointrange") +
  stat_summary(aes(y=m.1), 
               fun = mean, geom = "line") +
  ylab("outcome") +
...
aov <- aov(Risk_Beta ~ condition, data = Neuroethics_Judgement)
summary(aov)
...
ggplot(data_long, aes(condition, y=value, fill=variable)) +
  geom_boxplot()+
  scale_y_continuous("Beta", breaks=seq(-1, 1,.1), limits=c(-1, 1)) +
  theme(axis.text.x = element_text(angle=65, vjust=0.6)) +
  labs(title="", 
...
TEST <-arrange(multilevel.data, trial_index, user_id)

Fullcount <- multilevel.data %>%
        group_by(trial_index, type, condition, Stimulus, experimental_treatment_selected) %>%
        summarise(
          count = n()
        )

total <- multilevel.data %>%
        group_by(trial_index) %>%
        summarise(
          total = n()
        )

Yes.outcome <- filter(Fullcount, experimental_treatment_selected == "Yes to treatment")

Yes.outcome <- merge(Yes.outcome, total)

Yes.outcome  <- select(Yes.outcome, trial_index, count, total)

TESTT <- merge(TEST, Yes.outcome, 'trial_index')



#write.csv(TESTT,'LogitGCA.01.csv')


foo <- poly(unique(TESTT$trial_index), 3)

TESTT[,paste("ot", 1:3, sep="")] <- 
  foo[TESTT$trial_index, 1:3]


#Quasi-logistic

TESTT$elog <- with(TESTT,
       log((count+ 0.5) / (total-count+0.5)))

TESTT$wts <- with(TESTT,
       1/(count+ 0.5) + 1/(total-count + 0.5))

model.full <- lmer(elog ~ 
                      (ot1+ot2+ot3)*type*condition + 
                      (ot1+ot2+ot3 | user_id) + 
                      (ot1+ot2+ot3 | user_id:type:condition), 
                      control = lmerControl(optimizer = "bobyqa"),
                      data=TESTT, weights = 1/wts, REML=F)

summary(model.full)

ggplot(TESTT, aes(trial_index, elog, color = Type)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
  stat_summary(aes(y=fitted(model.full), linetype= Type), 
               fun = mean, geom = "line")+ 
  ylab("Outcome")+ 
